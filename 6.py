...
try:
    ...
    frappe.response["message"] = f"the get_all returned data ::: line 9 {user_list} "
    ...
    if not user_list:
        ...
        # Retrieve the password from the _Auth doctype
        user_auth = frappe.get_doc("_Auth", user_list[0].name)

        frappe.response["message"] = f"the get_doc returned data ::: line 20 {user_auth}....{password_input}"

        # Hash the input password
        hashed_input_password = frappe.utils.password.get_encrypted_password(password_input)

        # Verify the hashed password using Frappe's password utility
        if hashed_input_password == user_auth.password:
